...
try
{
	...
	ResultSet rs=stmt.executeQuery("select sum(total) from cart where email='"+emailid+"' and status='bill'");
	while(rs.next())
	{
		total=rs.getInt(1);
	}
	ResultSet rs2=stmt.executeQuery("select * from users inner join cart where cart.email='"+emailid+"' and cart.status='bill'");
	while(rs2.next())
	{
%>		
...
<div class="right-div-right"><h3>Email: <%out.println(emailid); %> </h3></div>
...
<h3>Product Details</h3>
  ...
  <% 
ResultSet rs1=stmt.executeQuery("select * from cart inner join addproduct where cart.p_id=addproduct.id and cart.email='"+emailid+"' and cart.status='bill'");
while(rs1.next())
  {
